...
class WC_POS_APIv2_Orders extends WC_POS_APIv2_Abstract {
	...
	public function register_additional_fields() {
		...
		register_rest_field( 'shop_order',
			...
			array(
				...
				'schema'					=> array(
					'description' => __( 'Cashier', 'arcane-woocommerce-pos' ),
					'type'				=> 'array',
		...
		register_rest_field( 'shop_order',
			...
			array(
				...
				'schema'					=> array(
					'description' => __( 'Payment Details', 'arcane-woocommerce-pos' ),
					'type'				=> 'array',
					'context'		 => array( 'view', 'edit' ),
					'arg_options' => array(
						'sanitize_callback' => 'rest_sanitize_request_arg',
	...
	public function get_payment_details( $response ) {
		...
		if( !empty( $payment['result'] ) && $payment['result'] === 'confirm' ){
			$payment['pi_secret'] = get_post_meta( $id, '_pos_payment_intent_secret', true );
		}
	...
	public function update_payment_details( $payment_details, $order ) {
		...
		if( $payment_method === 'stripe_terminal' ){
			if( !empty( $payment_details['terminal_id'] ) && empty( $payment_details['paid'] ) ){
				if( $order->get_status() === 'wc-cancelled' || $order->get_status() === 'cancelled' ){
					$order->update_status( 'pending' );
				}

				update_post_meta( $order->get_id(), '_pos_payment_terminal_id', $payment_details['terminal_id'] );
				update_post_meta( $order->get_id(), '_pos_payment_result', 'terminal_preprocess' );
				update_post_meta( $order->get_id(), '_pos_payment_message', '' );
			}else if( !empty( $payment_details['paid'] ) && $payment_details['paid'] === true ){
				$this->payment_success( $payment_method, $order, array() );

				$order->set_date_paid( current_time( 'timestamp', true ) );
				$order->set_date_completed( current_time( 'timestamp', true ) );
				$message = __( 'POS Transaction completed.', 'arcane-woocommerce-pos' );
			...
			$gateways[ $payment_method ]->retry_interval = 5; // Don't keep retrying a failed payment.
			$response = $gateways[ $payment_method ]->process_payment( $order->get_id() );

			if ( isset( $response['result'] ) && $response['result'] == 'success' ) {
				if( !empty( $response['payment_intent_secret'] ) && !empty( $response['redirect'] ) ){
					$this->payment_confirm( $order, $response['payment_intent_secret'] );
				}else{
				...
				$order->update_status( 'wc-failed', __( 'There was an error processing the payment', 'arcane-woocommerce-pos' ) );
			}
		}
	...
	private function payment_confirm( $order, $pi_secret ){
		update_post_meta( $order->get_id(), '_pos_payment_intent_secret', $pi_secret );
		update_post_meta( $order->get_id(), '_pos_payment_result', 'confirm' );
		update_post_meta( $order->get_id(), '_pos_payment_message', '' );
	}
	...
	private function payment_success( $gateway_id, $order, $response ) {
		...
		delete_post_meta( $order->get_id(), '_pos_payment_intent_secret' );
	...
	private function payment_failure( $gateway_id, $order, $response ) {
		...
		if ( empty( $response['messages'] ) ) {
			$response['messages'] = function_exists( 'wc_get_notices' ) ? wc_get_notices( 'error' ) : __( 'There was an error processing the payment', 'arcane-woocommerce-pos' );
		}
	...
	private function payment_redirect( $gateway_id, $order, $response ) {
		...
		if ( $offsite ) {
			...
			$message = __( 'You are now being redirected offsite to complete the payment. ', 'arcane-woocommerce-pos' );
			$message .= sprintf( __( '<a href="%s">Click here</a> if you are not redirected automatically. ', 'arcane-woocommerce-pos' ), $response['redirect'] );
		}
	...
	public function process_payment( $payment_details, $order ){
		...
		foreach($payment_details as $key => $value){
			...
			}else if($key == 'stripe_source'){
				$_POST['stripe_source'] = $value;
				$_POST['stripeSource'] = $value;
			}
		}
	}
}