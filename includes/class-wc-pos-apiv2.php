...
class WC_POS_APIv2{
	...
	public function __construct()
	{
		...
		add_filter('rest_request_before_callbacks', array($this, 'rest_update_filter'), 10, 3);
	...
	public function wc_api_authentication()
	{
		...
		if (!user_can($user->ID, 'access_woocommerce_pos'))
			$user = new WP_Error(
				...
				__('User not authorized to access WooCommerce POS', 'arcane-woocommerce-pos'),
				array('status' => 401)
			);
	...
	public function rest_update_filter( $response, $handler, $request ){
		$route = '/wc/v3/orders/(?P<id>[\d]+)';
		$path = $request->get_route();
		$match = preg_match( '@^' . $route . '$@i', $path, $matches );

		if ( ! $match || $request->get_method() !== 'PUT' ) {
			return $response;
		}
