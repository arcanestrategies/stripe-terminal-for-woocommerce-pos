...
class WC_POS_Settings {
	...
	const DB_PREFIX = 'arcane_woocommerce_pos_';
	...
	private function process_admin_settings(){
		...
		if(!isset($_GET['id']))
			return new WP_Error(
				...
				__( 'There is no settings id', 'arcane-woocommerce-pos' ),
				array( 'status' => 400 )
			);
		...
		if($count){
			...
			if(!isset($handlers[$id]))
				return new WP_Error(
					'woocommerce_pos_settings_error',
					sprintf( __( 'No handler found for %s settings', 'arcane-woocommerce-pos' ), $_GET['id']),
					array( 'status' => 400 )
				);
		...
		return new WP_Error(
			...
			__( 'Settings error', 'arcane-woocommerce-pos' ),
			array( 'status' => 405 )
		);
	}
