...
class WC_POS_Status {
	...
	private function test_wc_version() {
		$result = array(
			'title'	 => __( 'WC Version', 'arcane-woocommerce-pos' ),
		...
		if( ! $result['pass'] ){
			$result = array_merge( $result, array(
				'message' => sprintf( __( 'WooCommerce >= %s required', 'arcane-woocommerce-pos' ), WC_POS_Activator::WC_MIN_VERSION),
				'buttons' => array(
					array(
	...
	private function test_php_version() {
		...
		$result = array(
			'title'	 => __( 'PHP Version', 'arcane-woocommerce-pos' ),
		...
		if( ! $result['pass'] ){
			$result = array_merge( $result, array(
				'message' => sprintf( __( 'PHP >= %s required', 'arcane-woocommerce-pos' ), WC_POS_Activator::PHP_MIN_VERSION ),
				'buttons' => array(
					array(
	...
	private function test_wc_rest_api() {
		$result = array(
			'pass'		=> true,
			'title'	 => __( 'WC REST API', 'arcane-woocommerce-pos' ),
			'message' => __( 'API is active', 'arcane-woocommerce-pos' )
		);
		...
		if( get_option('woocommerce_api_enabled') !== 'yes' ) {
			...
			if( version_compare( WC()->version, '2.4', '>=' ) ){
				if( version_compare( WC()->version, '3.4', '>=' ) ){
					$href .= '&tab=advanced&section=legacy_api';
				}else{
					$href .= '&tab=api';
				}
			}
			...
			return array(
				'pass' => false,
				'message' => __('Access to the WooCommerce Legacy REST API is required', 'arcane-woocommerce-pos'),
				'buttons' => array(
					array(
						'href' => $href,
	...
	static public function permalinks_disabled(){
		...
		if( empty( $permalinks ) ) {
			return array(
				'pass'		=> false,
				'message' => __( '<strong>WooCommerce REST API</strong> requires <em>pretty</em> permalinks to work correctly', 'arcane-woocommerce-pos' ),
				'buttons' => array(
					array(
						'href'	=> admin_url('options-permalink.php'),
						'prompt'	=> __( 'Enable permalinks', 'arcane-woocommerce-pos' )
					)
				)
			);
		}
