...
class WC_POS_Template {
	...
	static public function get_external_js_libraries() {
		return defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG && file_exists( WC_POS_PLUGIN_PATH . 'assets/js/app.build.js' ) ? self::$external_libs[ 'debug' ] : self::$external_libs[ 'min' ];
	}
	...
	public function head() {
		...
		foreach( $wp_styles->queue as $style ){
			$real_style = $wp_styles->registered[$style];

			echo '<link rel="stylesheet" href="' . trim( $real_style->src ) . '" type="text/css" />' . "\n";
		}
	...
	public function footer() {
		$build = defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG && file_exists( WC_POS_PLUGIN_PATH . 'assets/js/app.build.js' ) ? 'build' : 'min';
		...
		$pos_options_json = 'var ' . str_ireplace('-', '_', WC_POS_PLUGIN_NAME . '-pos-app-params') . ' = ' . json_encode( apply_filters(WC_POS_PLUGIN_NAME . '-pos-app-params', $pos_options) ) . ';';
		echo $this->format_js( trim( $pos_options_json ) ) . "\n";
		...
		foreach( $wp_scripts->queue as $script ){
			$real_script = $wp_scripts->registered[$script];
			if( !empty( $real_script->extra ) && !empty( $real_script->extra['data'] ) ){
				echo $this->format_js( trim( $real_script->extra['data'] ) ) . "\n";
			}

			echo '<script src="' . trim( $real_script->src ) . '"></script>' . "\n";
		}
	}
	...
	static public function locate_template_file( $default_path = '' ) {
		$custom_path1 = str_replace( self::get_template_dir(), 'arcane-woocommerce-pos', $default_path );
	...
	private function load_extras(){
		...
		foreach( $gateways as $gateway ){
			if( strcasecmp( $gateway->enabled, 'yes' ) === 0 && method_exists( $gateway, 'payment_scripts' ) ){
