...
class WC_POS_Gateways_Card extends WC_Payment_Gateway {
  ...
  public function __construct() {
    $this->id           = 'pos_card';
    $this->title        = __( 'Card', 'arcane-woocommerce-pos' );
    $this->description  = '';
  ...
  public function payment_fields() {
    ...
    echo '
      <div class="form-row " id="pos-cashback_field">
        <label for="pos-cashback" class="">'. __('Cashback', 'arcane-woocommerce-pos') .'</label>
        <div class="input-group">
        '. $left_addon .'
          <input type="text" class="form-control" name="pos-cashback" id="pos-cashback" placeholder="" maxlength="20" data-value="0" data-numpad="cash" data-label="'. __('Cashback', 'arcane-woocommerce-pos') .'">
        '. $right_addon .'
        </div>
      </div>
    ';
  ...
  public function process_payment( $order_id ) {
    ...
    if( $cashback !== 0 ) {
      ...
      $item_id = wc_add_order_item( $order_id, array(
        'order_item_name' => __('Cashback', 'arcane-woocommerce-pos'),
        'order_item_type' => 'fee'
      ) );
  ...
  public function calculate_cashback( $order_id ) {
    ...
    // construct message
    if( $cashback ) {
      $message = __('Cashback', 'arcane-woocommerce-pos') . ': ';
