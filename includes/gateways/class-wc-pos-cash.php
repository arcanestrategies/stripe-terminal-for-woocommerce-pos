...
class WC_POS_Gateways_Cash extends WC_Payment_Gateway {
  ...
  public function __construct() {
    $this->id          = 'pos_cash';
    $this->title       = __( 'Cash', 'arcane-woocommerce-pos' );
    $this->description = '';
  ...
  public function payment_fields() {
    ...
    echo '
      <div class="form-row" id="pos-cash-tendered_field">
        <label for="pos-cash-tendered" class="">'. __('Amount Tendered', 'arcane-woocommerce-pos') .'</label>
        <div class="input-group">
        '. $left_addon .'
          <input type="text" class="form-control" name="pos-cash-tendered" id="pos-cash-tendered" maxlength="20" data-numpad="cash" data-label="'. __('Amount Tendered', 'arcane-woocommerce-pos') .'" data-placement="bottom" data-value="{{total}}">
        '. $right_addon .'
        </div>
      </div>
    ';
  ...
  public function calculate_change( $order_id ) {
    ...
    // construct message
    if( $tendered && $change ) {
      $message = __('Amount Tendered', 'arcane-woocommerce-pos') .': ';
      $message .= wc_price($tendered) .'<br>';
      $message .= _x('Change', 'Money returned from cash sale', 'arcane-woocommerce-pos') .': ';
