...
<head>
  <meta charset="utf-8">
  <title><?php _e( 'Receipt', 'arcane-woocommerce-pos' ); ?></title>
  <style>
    /* Reset */
    * {
...
<div class="order-addresses">
  <div class="billing-address">
    {{formatAddress billing_address title="<?php _e( 'Billing Address', 'arcane-woocommerce-pos' ); ?>"}}
  </div>
  <div class="shipping-address">
    {{formatAddress shipping_address title="<?php _e( 'Shipping Address', 'arcane-woocommerce-pos' ); ?>"}}
  </div>
</div>
<table class="order-info">
  <tr>
    <th><?php _e( 'Order Number', 'arcane-woocommerce-pos' ); ?></th>
    <td>{{order_number}}</td>
  </tr>
  <tr>
    <th><?php _e( 'Order Date', 'arcane-woocommerce-pos' ); ?></th>
    <td>{{formatDate completed_at format="MMMM Do YYYY, h:mm:ss a"}}</td>
  </tr>
  {{#if cashier}}
  <tr>
    <th><?php _e( 'Cashier', 'arcane-woocommerce-pos' ); ?></th>
    <td>{{cashier.first_name}} {{cashier.last_name}}</td>
  </tr>
  {{/if}}
  <tr>
    <th><?php _e( 'Payment Method', 'arcane-woocommerce-pos' ); ?></th>
...
<table class="order-items">
  <thead>
    <tr>
      ...
      <th class="qty"><?php _ex( 'Qty', 'Abbreviation of Quantity', 'arcane-woocommerce-pos' ); ?></th>
  ...
  <tfoot>
    <tr class="subtotal">
      <th colspan="2"><?php _e( 'Subtotal', 'arcane-woocommerce-pos' ); ?>:</th>
      <td colspan="1">{{{money subtotal}}}</td>
    </tr>
    {{#if has_discount}}
      <tr class="cart-discount">
        <th colspan="2"><?php _e( 'Discount', 'arcane-woocommerce-pos' ); ?>:</th>
      ...
      {{#if itemized}}
        {{#each tax_lines}}
          {{#if has_tax}}
            <tr class="tax">
              <th colspan="2">
                {{#if ../../incl_tax}}<small>(<?php _ex( 'incl.', 'abbreviation for includes (tax)', 'arcane-woocommerce-pos' ); ?>)</small>{{/if}}
                {{title}}:
              </th>
              <td colspan="1">{{{money total}}}</td>
            </tr>
    ...
    <tr class="pos_cash-tendered">
      <th colspan="2"><?php _e( 'Amount Tendered', 'arcane-woocommerce-pos' ); ?>:</th>
      <td colspan="1">{{{money payment_details.method_pos_cash.tendered}}}</td>
    </tr>
    <tr class="pos_cash-change">
      <th colspan="2"><?php _ex('Change', 'Money returned from cash sale', 'arcane-woocommerce-pos'); ?>:</th>
